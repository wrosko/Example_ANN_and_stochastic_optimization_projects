...
//==========================================================
/*
This program is a modification of Peter Cole's original "TestGAc-struct.pc." written by Wade Rosko.
...
// data file protocol functions
void initDataFile();
void writeDataFileForGenotype(int genotypeIndex);
void writeDataFileForCrossoverGenotype(int genotypeIndex);
void closeDataFile();
...
//==========================================================
//This is the function starting a new datafile. It deletes any pre-existing data file in the robot's memory. The datafile is only storing three values. It will record the generation,
//the genotype index (which individual in the generation), and the fitness value for the respective individual. 
//==========================================================


void initDataFile() // what follows defines the function that initializes the new data file, sets the parameters defining its size and shape; and fills in the first line with column titles
{


	Delete("dataFile.txt", nIOResult); // clears and resets the data file so it can be used by the program
	OpenWrite(hFileHandle, nIOResult, sFileName, nFileSize); // opens the files and sets the parameters needed to collect, record, and store data in the program (see above)

	string colTitle[3] = {"Generation,","genotypeIndex,", "Fitness,"};
	// an array that defines the column titles to write in as the first line in the data file

	for (int i = 0; i < 3; i++)
	{
		WriteText(hFileHandle, nIOResult, colTitle[i]);
	}
	// a "for loop" that fills in the column titles as objects in a 15 dimensional array (...or writes the first line of the data file)

	WriteByte(hFileHandle, nIOResult, 13);	// a "carriage return" command to start a new line
	WriteByte(hFileHandle, nIOResult, 10);	// a "left justify" command to move back to the left side of the line
	WriteByte(hFileHandle, nIOResult, 13);
	WriteByte(hFileHandle, nIOResult, 10);
  ...
  for (int k = 0; k < 10; k++)
	{
		...
		writeDataFileForGenotype(M+k);//Write data file for genotype
...
//==========================================================
//This function is similar to the other datafiles we have worked with. If it is the first generation then it writes the values to the first 
//
//==========================================================
...
void writeDataFileForGenotype(int genotypeIndex)
{
	...
	if (firstTimeArray[genotypeIndex] == false)
	{
		string genotypeString = "";
		genotypeIndex = genotypeIndex - M;
		genotypeString = genomes[genotypeIndex].chromoArray[0] + genomes[genotypeIndex].chromoArray[1] + genomes[genotypeIndex].chromoArray[2] + genomes[genotypeIndex].chromoArray[3] + genomes[genotypeIndex].chromoArray[4] + genomes[genotypeIndex].chromoArray[5] + genomes[genotypeIndex].chromoArray[6] + genomes[genotypeIndex].chromoArray[7];

		WriteByte(hFileHandle, nIOResult, 13);
		WriteByte(hFileHandle, nIOResult, 10);

		WriteText(hFileHandle, nIOResult, genotypeString);
	...
	firstTimeArray[genotypeIndex] = true;
	//-----------------------------------------------

	string genotypeIndexString = "";
	string fitnessValueString = "";
	string generationString = "";

	StringFormat(generationString,"%d,",x);
	StringFormat(genotypeIndexString, "%d,", genotypeIndex);
	StringFormat(fitnessValueString, "%d,", genomes[genotypeIndex].fitnessValue);

	WriteText(hFileHandle, nIOResult, generationString);
	WriteText(hFileHandle, nIOResult, genotypeIndexString);
	WriteText(hFileHandle, nIOResult, fitnessValueString);
	WriteByte(hFileHandle, nIOResult, 13);
	WriteByte(hFileHandle, nIOResult, 10);
...
void writeDataFileForCrossoverGenotype(int genotypeIndex)
{
	...
	if (firstTimeCrossArray[genotypeIndex] == false)
	{
	  genotypeIndex = genotypeIndex - N;
		string genotypeString = "";
		genotypeString = crossoverGenomes[genotypeIndex].chromoArray[0] + crossoverGenomes[genotypeIndex].chromoArray[1] + crossoverGenomes[genotypeIndex].chromoArray[2] + crossoverGenomes[genotypeIndex].chromoArray[3] + crossoverGenomes[genotypeIndex].chromoArray[4] + crossoverGenomes[genotypeIndex].chromoArray[5] + crossoverGenomes[genotypeIndex].chromoArray[6] + crossoverGenomes[genotypeIndex].chromoArray[7];
	...
	firstTimeCrossArray[genotypeIndex] = true;
	//-----------------------------------------------
	...
	StringFormat(generationString,"%d,",x);
	StringFormat(genotypeIndexString, "%d,", genotypeIndex);
	StringFormat(fitnessValueString, "%d,", crossoverGenomes[genotypeIndex].fitnessValue);
...
void closeDataFile()
{
	nxtDisplayCenteredBigTextLine(3, "FileClosed");	 // display on line 3, a big, centered, message
...
void FITNESS()
{
	...
	one_minus_i = 1 - MaxProx; //Encouragge obstacle avoidance
...
void rankGenotypes()
{
	...
	for (int k = 1; k < 10; k++)
	{
			...
			if (k == 9)
		{
			...
			WriteText(hFileHandle, nIOResult, fittestString);
